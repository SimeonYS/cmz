...
class CmzPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('cmz.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cmz`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cmz WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cmz (date, title, link, content)"
